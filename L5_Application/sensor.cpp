...
{     static QueueHandle_t sensor_data_q = scheduler_task::getSharedObject("sensor_queue");
        ...
        LE.off(3);
        pin_number =2;
        if(!xQueueSend(sensor_data_q,&pin_number, 1))
        {
                    puts("Failed to send item to queue");
                }
}
...
bool sensorTask::init(void)
    {
        ...
        //create queue
        sensor_data_q = xQueueCreate(100, sizeof(uint32_t));
...
bool sensorTask::run(void *p)
    {
    if( xQueueReceive(sensor_data_q, &pin_number,100))
        {
                  delay_us(mydel);
